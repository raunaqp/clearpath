...
- No bombastic claims of replacing experts.
- No "AI" hype — phrase it as "workflow architecture, not generative AI invention" (slide 6).
...
**Layout.** 4-column horizontal flow with arrow separators. Each column = one stage: Intake / Risk Card / Tier B Wizard / Draft Pack. Each column shows a stage label (numbered 01-04), stage name, one-line description, and a small mockup of the actual screen. Below the flow: a teal-tinted disclaimer band.

**Mockups embedded.**
- **Intake mockup**: drag-drop zone (teal-50 background, dashed teal border) + "✓ cardiorhythm-deck.pdf · 4.2 MB" line beneath
- **Risk Card mockup**: large "62 / 100" readiness score + "Class C SaMD" badge + 3 top-gap bullets
- **Tier B Wizard mockup**: 6-dot progress + B3 step + predicate input rows
- **Draft Pack mockup**: section list with status icons (✓ / ◐ / ○)

**Disclaimer copy (verbatim).** "ClearPath structures readiness workflows. It does not replace regulatory consultants or guarantee approvals. Every Draft Pack section is editable and intended as a starting point for review by qualified regulatory experts."

**Presenter note.** "These are the four touchpoints. Wednesday's demo walks each one with a real founder profile — CardioRhythm, a Class C SaMD."
...
**Layout.** Subtitle → 2-column: left is the 5-layer system stack (each layer is a coloured bar with name + description + monospace example); right is a single large product mockup of the Tier A wizard Q2 with a caption.
...
5. Draft generation (indigo)

Each separated by a `▾` arrow. The teal escalation in colour conveys depth-of-reasoning.

**Mockup on right.** Tier A Q2 — "How much does your product influence clinical decisions?" — three RadioCards, middle one (Drives) selected. Caption: "This single answer drives downstream class estimation, ACP applicability, and which sections of the Draft Pack require clinical evidence depth."

**Presenter note.** "This is the slide where we want to give your team confidence that the system isn't generative AI guessing. Each answer is a deterministic input into a rule structure. The AI is used for extraction and drafting at the edges — the core reasoning is encoded."
...
2. AI extraction (monospace field-extract mockup)
3. Tier A — prefilled (wizard mockup with "4 of 7 prefilled" badge)
4. Risk Card (readiness score + recommended path mockup)
5. Tier B — prefilled (wizard mockup with "B1, B2, B5, B6 prefilled" badge)
6. Draft Pack (12-section list mockup, with `Sprint 3+` corner tag)

**Closing line copy (verbatim).** "Compresses initial documentation readiness significantly. The Draft Pack is a starting point for review by qualified regulatory experts — not a substitute for one."

**Critical:** The closing line is *not* "30 min vs 3-6 months" per the user's anti-overclaim guidance. The mockup #6 has the explicit `Sprint 3+` corner tag because Draft Pack is in active development.
...
**Layout.** Subtitle → 2-column: left is a 5-item pain list with numbered amber bullets; right is a gray "Indian medtech context" card with two stats + a closing block on BIG / Venture Center fit.

**Five pain points** (left). All sourced from CDSCO FAQ patterns + ORF research + the bible's own gap analysis:
1. Founders discover regulation too late
2. TRL ladder and regulatory readiness are disconnected
3. Sequencing errors are common (MD-22 without MD-13; MD-7 without MD-27)
4. Consultant-heavy ecosystem
5. Incubator cohorts need structured pre-consultant prep

**Two stats** (right). ~1,000 pure medical device startups (3,000-6,000 broader medtech+health-tech) [BioSpectrum Asia, Accorp]; ~$12B → $50B market FY24 → 2050 (~15% CAGR), 600+ devices under mandatory registration [ORF, Accorp].

**Closing block** (right, below stats). "BIG / incubator fit. BIRAC's BIG cohort scope explicitly includes medical devices, diagnostics, and drug-delivery — exactly the founders who hit MDR-2017 navigation at TRL 5-7. Venture Center is BIRAC's 3rd Regional Centre."

**Critical:** Not a TAM slide. Numbers serve to anchor scale, not justify ambition. Pain list is the dominant content.

**Presenter note.** "We deliberately led with founder pain rather than market size. The numbers on the right are calibration, not pitch."
...
**Presenter note.** "Five concrete commitments. Where something is shipped, we say so. Where something is in-progress (DPDP Phase 3 by May 2027), we say that too. Founders share regulated information through ClearPath; we owe them this specificity."
...
| # | Stage | Time |
|---|---|---|
| 01 | Pitch deck upload + automatic extraction | 5 min |
| 02 | Tier A wizard end-to-end | 10 min |
| 03 | Risk Card review | 3 min |
| 04 | Tier B wizard — prefill + completion | 10 min |
| 05 | Draft Pack output structure | 5 min |
...
**Profile.** Name + role + short paragraph + email + programme line.

**Four context blocks.**
- Why this problem
- Adjacent ecosystem familiarity
- Build posture
- Sponsor / programme context

**Critical:** Avoid resume-style bullet list. Context blocks are short paragraphs in body-prose, not bullets. Each starts with a teal eyebrow tag.

**Presenter note.** "Brief context — we are not selling our team. We are giving you enough to know who you'll be talking to on Wednesday and where this work sits institutionally."
...
1. **Live screenshots vs CSS mockups.** Mockups currently match the production UI vocabulary but are HTML/CSS-built, not photographic. If a higher-fidelity revision is wanted post-Wed, capturing real screenshots of the wizard + intake + Risk Card flows would tighten visual realism.
2. **Slide 13 founder details.** Currently uses what's verifiable from the project context. If specific institutional affiliations (ARTPARK, IISc, etc.) apply, they can be added in.
3. **Slide 8 stat sourcing depth.** Numbers are sourced from public secondary research. A primary-source cohort statistic from Venture Center itself (if shareable) would be stronger.
