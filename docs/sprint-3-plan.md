...
- **4.11** `generate-draft-pack-v2-live.ts` test-script cleanup — set `tier_choice='draft_editor'` and `amount_inr=2499` on auto-create; better, refactor to call `/api/cashfree/create-order` + admin-mark-paid path rather than direct INSERT so the script can't bypass production invariants. Day-5 EOD test order `92e73de8` for assessment `0be5a3db` was created with `tier_choice=null` + `amount_inr=499`, leaving the order stuck at `status='verified'` indefinitely on `/upgrade/`. Not a prod risk (production payment flow makes `verified + null tier_choice` unreachable per webhook + create-order analysis), but cleans up the local-test footgun  ·  0.5 day
- **4.12** UI hardening on `/upgrade/[id]` for null `tier_choice` (defensive belt-and-suspenders against future script bugs, admin slips, or migration mishaps). When `status='verified'` AND `tier_choice IS NULL`, show a clear "order configuration incomplete — contact support" instead of an indefinite "generating now…" spinner. Production code path doesn't produce this state today, but the hardening is a low-cost safety net  ·  0.5 day
- **4.13** Code-duplication architectural guardrail — audit script for inline copies of `lib/` helpers + ESLint rule prohibiting function-definition duplication across the repo. Lesson from the Day-5 EOD `shortDeviceName` hero-truncation bug, where four copies of the same helper had grown across the codebase (one wrong; the sample script used `displayName` from `lib/wizard/` instead of the trigger's `shortDeviceName`). The architectural rule — any helper used by ≥2 call sites lives in `lib/` as a shared export from day one, no inline copies, no "kept in sync manually" comments — needs tooling to enforce, not just memory. Memory: `feedback_no_inline_helper_copies.md`  ·  1 day
- **4.14** `/api/upgrade/status` 44-minute hung response — Day-6 prod-deployed-path observation: a single GET `/api/upgrade/status?assessment_id=0be5a3db…` returned 200 after **44.1 minutes**. The endpoint is a status poll used by the StatusPanel during generation; a single request blocking for 44 minutes is a real stability concern even though it eventually completed. Likely candidates: (a) Supabase connection pool exhaustion holding the query open, (b) Fluid Compute keeping the function alive across multiple poll cycles via long-running `after()` work, (c) some upstream Anthropic / generator call inheriting the request context. Demo-day risk: if a real customer happens to poll while this is happening, their browser hangs for tens of minutes. Investigate + add a hard timeout (e.g., 10s with a clear "still generating" response) + observability on long-tail latencies  ·  1.5 days
